...
local function getLeftTime()
    local left = uiReference.audioChannel.clip.length * (1-uiReference.sliderProgress.value)
    return string.format("%02d:%02d", math.floor(left/60), math.floor(left%60))
end
...
local function refreshSubtitle()
    ...
    for k, v in pairs(subtitles) do
        if v["timestamp"] < (uiReference.sliderProgress.value * uiReference.audioChannel.clip.length)* 1000 then
            subtitle = v["txt"]
        end
    end
...
local function rewind()
    isPlaying = false
    uiReference.animationCover:Stop()
    uiReference.audioChannel:Stop()
    uiReference.objPlayButton:SetActive(true)
    uiReference.objPauseButton:SetActive(false)
    uiReference.sliderProgress.value = 0
    uiReference.audioChannel.time = 0
...
local function run()
    ...
    preReadMusicCallback.onSuccess = function()
        -- 加载音频
        local audioClip = g_archiveReader:ReadAudioClip("music.ogg")
    ...
    g_archiveReader:PreReadAudioClipAsync("music.ogg", preReadMusicCallback)

    -- 预加载伴奏
    local preReadAccompanimentCallback = g_archiveReader:NewPreReadCallback()
    preReadAccompanimentCallback.onSuccess = function()
        -- 加载音频
        local audioClip = g_archiveReader:ReadAudioClip("accompaniment.ogg")
    ...
    g_archiveReader:PreReadAudioClipAsync("accompaniment.ogg", preReadAccompanimentCallback)
end
...
local function update()
    ...
    if uiReference.audioChannel.time >= uiReference.audioChannel.clip.length then
        rewind()
        return
    end
