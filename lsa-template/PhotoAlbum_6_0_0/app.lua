...
local function viewPhoto(_texture, _cell, _index)
    ...
    -- 显示描述
    uiReference.objDescription.transform:Find("Text"):GetComponent(typeof(ugui.Text)).text = config.Desc["img#".._index..".jpg"]["en_US"]
end
...
local function addScrollContainer()
    ...
    loadCoroutine = util.cs_generator(function()
        ...
        for i = 1,config.Count do
            ...
            toggle.onValueChanged:AddListener(function(_toggled)
...
local function addToolbar()
    ...
    ,unity.Vector2(1,1)
    ,unity.Vector2(0,0)
    ,unity.Vector2(0,-style.toolbar_width*1.5)
    ,unity.Vector2(0.5, 1)
    )
    uiReference.slider = slider
    slider.minValue = 1
    ...
    ,unity.Vector2(0.5,0)
    ,unity.Vector2(0,0)
    ,unity.Vector2(style.toolbar_width, style.toolbar_width)
    ,unity.Vector2(0.5, 0)
    )
